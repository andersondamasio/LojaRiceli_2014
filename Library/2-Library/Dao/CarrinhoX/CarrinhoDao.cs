...
{
    internal sealed class CarrinhoDao : Repositorio<Carrinho>
    {
        ...
        public List<CarrinhoDto> SelectCarrinho(int loj_id, string car_sessionId)
        {
            List<CarrinhoDto> carrinho = (from car in Select()
                                          ...
                                          select new CarrinhoDto
                                          {
                                              ...
                                              proSku_disponivel = car.ProdutoSku.proSku_disponivel == true &&
                                                                 car.ProdutoSku.proSku_bloquear == false &&
                                                                 car.ProdutoSku.Produto.pro_bloquear == false &&
                                                                 car.ProdutoSku.Produto.Produto_Grupo.Where(s => s.Grupo.gru_bloquear == false && s.Grupo.gru_subBloquear == false).Count() > 0,                                          
