...
using _2_Library.Dao.Site.CupomX;
...
namespace _2_Library.Dao.Site.CarrinhoX
{
    public class CarrinhoTd
    {
        public void InsertCarrinho(string loj_dominio, CarrinhoDto carrinhoDto)
        {
            ...
            using (CarrinhoDao rep = new CarrinhoDao())
            {
               ...
               /* var auditConfiguration = AuditConfiguration.Default;
                ...
                AuditConfiguration.Default.IsAuditable<Carrinho>();

                auditConfiguration.IsAuditable<Carrinho>().DisplayMember(t => t.car_quantidade);

                var audit = rep.lojaEntities.BeginAudit();
        ...
        public List<CarrinhoDto> SelectCarrinho(string loj_dominio, string car_sessionId, int ent_cep)
        {
            int loj_id = new LojaTd().SelectLoja(loj_dominio).loj_id;
            ...
            using (CarrinhoDao rep = new CarrinhoDao())
            {
                ...
                carrinhoTd = (from car in carrinho
                              select new CarrinhoDto
                              {
                                  ...
                                  entregaDto = Produto_GrupoUtils.VerificaEntrega(car.entregaDto, ent_cep),
        ...
        /// <returns></returns>
        public CarrinhoTotaisDto SelectCarrinhoTotais(List<CarrinhoDto> carrinho, string ent_cep, string cup_chave, int? cli_id)
        {
            int ent_cepDest = 0;
            if (!string.IsNullOrEmpty(ent_cep) && Validacao.ValidaInteiro(ent_cep))
                ent_cepDest = Convert.ToInt32(ent_cep);
                
            if(carrinho == null)
                carrinho = SelectCarrinho(null, null, Convert.ToInt32(ent_cepDest));
            ...
            if (carrinho.Count != 0)
            {
                ...
                //total do prazo da entrega fixado(pega o maior prazo dentre os produtos)
                if (carrinhoDtoEnt_bloquear.Where(s => s.entregaDto.ent_prazo.HasValue).Count() > 0)
                    prazoEntregaFixoTotal = carrinhoDtoEnt_bloquear.Where(s => s.entregaDto.ent_prazo.HasValue).OrderByDescending(s => s.entregaDto.ent_prazo).FirstOrDefault().entregaDto.ent_prazo;
                ...
                if (!string.IsNullOrEmpty(ent_cep))
                {
                    ...
                    if (carrinho.Count == 1)
                    {
                        ...
                        if (carrinhoDto.entregaDto.ent_bloquear == false)
                        {
                            ...
                            if (carrinhoDto.entregaDto.ent_calculaPrazoExterno && carrinhoDto.entregaDto.ent_calculaValorExterno)
                            {
                                ...
                                correioDto = new CorreioTd().SelectCorreioCalcPrecoPrazo(null, carrinho, carrinhoMedidas, null, null, ent_cep);
                                ...
                                if (carrinhoDto.entregaDto.ent_calculaPrazoExterno && !carrinhoDto.entregaDto.ent_calculaValorExterno)
                                {
                                    correioDto = new CorreioTd().SelectCorreioCalcPrazo(null, null, null, ent_cep);
                                    ...
                                    {
                                        CarrinhoMedidas carrinhoMedidas = SelectCarrinhoSomaMedidas(carrinho);
                                        correioDto = new CorreioTd().SelectCorreioCalcPreco(null, carrinho, carrinhoMedidas, null, null, ent_cep);
                                        ...
                                        if (!carrinhoDto.entregaDto.ent_calculaPrazoExterno && !carrinhoDto.entregaDto.ent_calculaValorExterno)
                                        {
                                            entregaValorTotal = carrinhoDto.entregaDto.ent_valor;
                                            entregaPrazoTotal = carrinhoDto.entregaDto.ent_prazo;
                                            correioDto = new CorreioTd().SelectCorreioLocalidade(ent_cep);
                                        }
                        }
                        else
                        {
                    ...
                    {
                        //se o carrinho tem mais de 1 item
                        //ent_bloquear == false
                        ...
                        if (carrinho.Where(s => (s.entregaDto.ent_bloquear == false) && s.entregaDto.ent_calculaPrazoExterno && s.entregaDto.ent_calculaValorExterno).Count() == carrinho.Count)
                        {
                            ...
                            //todos os produtos com apenas o prazo calculado externamente
                            if (carrinho.Where(s => (s.entregaDto.ent_bloquear == false) && s.entregaDto.ent_calculaPrazoExterno && !s.entregaDto.ent_calculaValorExterno).Count() == carrinho.Count)
                            {
                                correioDto = new CorreioTd().SelectCorreioCalcPrazo(null, null, null, ent_cep);

                                entregaValorTotal = entregaValorFixoTotal;
                                entregaPrazoTotal = Convert.ToInt32(correioDto.co_prazoEntrega);  

                            }
                            else
                                //todos os produtos com apenas o valor calculado externamente
                                if (carrinho.Where(s => (s.entregaDto.ent_bloquear == false) && !s.entregaDto.ent_calculaPrazoExterno && s.entregaDto.ent_calculaValorExterno).Count() == carrinho.Count)
                                {
                                    CarrinhoMedidas carrinhoMedidas = SelectCarrinhoSomaMedidas(carrinho);
                                    correioDto = new CorreioTd().SelectCorreioCalcPreco(null, carrinho, carrinhoMedidas, null, null, ent_cep);
                                    ...
                                    entregaValorTotal = Convert.ToDecimal(correioDto.co_valor);
                                    entregaPrazoTotal = prazoEntregaFixoTotal;     
                                }
                                else
                                    //pega o prazo e valor calculado internamente na entrega fixa
                                    if (carrinho.Where(s => (s.entregaDto.ent_bloquear == false) && !s.entregaDto.ent_calculaPrazoExterno && !s.entregaDto.ent_calculaValorExterno).Count() == carrinho.Count)
                                    ...
                                    {
                                        //com entrega bloqueada, calcula externamente o preco e prazo
                                        if (carrinho.Where(s => s.entregaDto.ent_bloquear == true).Count() > 0)
                                        {
                                            ...
                                            //calcula apenas o preco externamente
                                            if (carrinho.Where(s => s.entregaDto.ent_calculaValorExterno).Count() > 0)
                                            {
                                                ...
                                                //calcula apenas prazo externamente
                    ...
                    geralTotal = (geralTotal < 0 ? 0 : geralTotal)  + (entregaValorTotal ?? 0);
                ...
                ParcelamentoParcelaDto parcelamentoParcelaSemJurosDto = parcelamentoDto.parcelamentoParcelaDto.Where(s=>s.parcPar_percentualJuro == 0).OrderByDescending(s => s.parcPar_quantidade).FirstOrDefault();
                ParcelamentoParcelaDto parcelamentoParcelaComJurosDto = parcelamentoDto.parcelamentoParcelaDto.Where(s => s.parcPar_percentualJuro != 0).OrderByDescending(s => s.parcPar_quantidade).FirstOrDefault();
               
                string condicao = string.Empty;

                if (parcelamentoParcelaSemJurosDto != null)
                    condicao += parcelamentoParcelaSemJurosDto.parcPar_quantidade + " x de " + parcelamentoParcelaSemJurosDto.parcPar_valor.ToString("c")+" sem juros";

                if (parcelamentoParcelaComJurosDto != null)
                    condicao += "<br/>ou em até "+parcelamentoParcelaComJurosDto.parcPar_quantidade + " x de " + parcelamentoParcelaComJurosDto.parcPar_valor.ToString("c");
